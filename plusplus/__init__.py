...
def _patch_increment(region, func):
    n_removed = 3

    if _is_pytest_intermediate_value_capturing(region[1:3]):
        # Remove capturing of the LOAD_ATTR/BINARY_SUBSCR result
        region[1:3] = []
        n_removed += 2
    if _is_pytest_intermediate_value_capturing(region[2:4]):
...
def _make_syntax_error(message, lineno, func):
    return SyntaxError('File "{}", line {}, in {}: {}'.format(
        func.__code__.co_filename, lineno, func.__name__, message))


def _is_pytest_intermediate_value_capturing(region):
    """
